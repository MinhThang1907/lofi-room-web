...
// Create Socket.IO server without custom path
...
io.on("connection", (socket) => {
  ...
  socket.on("join-room", async (data) => {
    ...
    try {
      ...
      if (socket.currentRoom && socket.currentRoom !== roomId) {
        ...
        await handleUserLeaveRoom(socket, socket.currentRoom);
      }
      ...
      // Check if user already exists in room state
  ...
  socket.on("send-message", async (data) => {
    ...
    console.log(
      `💬 Message from ${socket.user.name} in room ${roomId}: ${message}`
    );
  ...
  socket.on("user-move", async (data) => {
    ...
    if (socket.currentRoom === roomId && rooms.has(roomId)) {
      ...
      if (user) {
        ...
        // Broadcast position update to other users (not sender)
  ...
  socket.on("disconnect", (reason) => {
    ...
    if (socket.currentRoom) {
      handleUserLeaveRoom(socket, socket.currentRoom);
    }
  });
...
async function handleUserLeaveRoom(socket, roomId) {
  if (rooms.has(roomId)) {
    const roomState = rooms.get(roomId);
